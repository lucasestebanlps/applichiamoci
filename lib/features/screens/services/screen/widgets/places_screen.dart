...
class PlacesScreen extends StatelessWidget {
  ...
  const PlacesScreen(
      {Key? key, required this.service, required this.nameService})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    if (placesController.placesForService.isEmpty ||
        placesController.currentService != service) {
      placesController.fetchPlacesForServices(service);
    }
          ...
          children: [
            Padding(
              ...
              child: DropdownButton<String>(
                value: placesController.currentService,
                items: placesController.services.map((String value) {
                ...
                onChanged: (newValue) {
                  placesController.setCurrentService(newValue);
                },
              ),
            ),
            Expanded(
              child: Padding(
                padding: const EdgeInsets.all(LSizes.sm),
                child: _buildPlacesList(placesController),
              ),
            ),
          ],
        );
  ...
  Widget _buildPlacesList(PlacesController placesController) {
    ...
    } else if (placesController.placesForService.isEmpty) {
      return LErrorCenteredText(
        icon: Icons.warning,
        text: tr(LocaleKeys.noLogoAvailable),
      );
    } else {
      List<PlaceModel> places = placesController.filteredAndSortedPlaces;
